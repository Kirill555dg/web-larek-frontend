...
export default class OrderModel extends Model<Record<string, unknown>> {
  ...
  updateItems(): void {
    ...
    this.order.total = cartItems.reduce((sum, item) => sum + (item.price ?? 0), 0);

    this.emitChanges("order:update", { field: "items", value: this.order.items });
    this.emitChanges("order:update", { field: "total", value: this.order.total });
  }
  ...
  confirmOrder(): void {
    if (!this.isOrderValid()) {
      this.emitChanges("order:submit-error", { message: "Не все поля заполнены" });
      return;
    }
