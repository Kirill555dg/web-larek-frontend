...
export default class ProductModel extends Model<Record<string, unknown>> {
  ...
  async loadProducts(): Promise<void> {
    try {
      ...
      this.emitChanges("product:list", { products: Array.from(this.products.values()) });
    } catch {
      this.emitChanges("product:load-error", { message: "Ошибка загрузки товаров" });
    }
  }
  ...
  async loadProduct(id: string): Promise<void> {
    ...
    try {
      ...
      if (product) {
        ...
        this.emitChanges("product:load-error", { message: "Товар не найден" });
      }
    } catch {
      this.emitChanges("product:load-error", { message: "Ошибка загрузки товара" });
    }
  }
